...
namespace Backend.ChatListener
{
	...
	public class TwitchIrcClient : ITwitchIrcClient
    {
        ...
        public void Connect()
        {
            Connect("justinfan54555","password");
        }
        ...
        private async void ReadMsg(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                if (msg is null)
                {
                    throw new ArgumentNullException("Connection lost");
                }
        ...
        private async Task<string?> ReadLine(CancellationToken token)
        {
        ...
        public event MessageEventHandler OnAction;
        public event MessageEventHandler OnChat;
        public event MembershipEventHandler OnJoin;
        public event MembershipEventHandler OnPart;
        public event ConnectEventHandler OnConnect;
        public event RawMessageEventHandler OnRawMsg;
    }
    ...
    public delegate void RawMessageEventHandler(object sender, RawMsgEventArgs rawMsg);
    public delegate void MembershipEventHandler(object sender, MembershipEventArgs membership);
    public delegate void ConnectEventHandler(object sender, EventArgs empty);
}